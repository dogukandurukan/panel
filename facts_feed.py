...
MIN_LEN = 90          # bundan kısa özetler "bilgi" sayılmayacak kadar cılız
...
def fetch_summary(session, topic):
    url = API + urllib.parse.quote(topic.replace(" ", "_"), safe="")
    ...
    return {
        "text": text,
        "title": d.get("titles", {}).get("normalized") or d.get("title") or topic,
